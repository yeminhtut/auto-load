<?php 
	$interest_post_list_two = 'Bangkok';
	$interest_post_list_two_per_page = 9;

	$interest_post_list_two_content  = '<section id="'.$id_category.'" class="homepage-widget widget-type-3 '.esc_attr( $type ).' ' .esc_attr( $el_class ).''.esc_attr( $css_class ).'">';
            $args_posts_list_3 = array(
				'post_type' 			=> 'post',
				'post_status' 			=> 'publish',
				'category__and' 		=> array(27),//sports id 
				'posts_per_page' 		=> absint( $interest_post_list_two_per_page )
			);
        	...
        	$interest_post_list_two_content .= '<div class="section-header"><h3 style="font-size:35px;margin-bottom:20px;line-height:56px;border-bottom: solid 2px #ddd;">Bangkok</h3></div>';
